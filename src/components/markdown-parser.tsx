...
const components = {
  ...
  h2: ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h2
      className={cn(
        "text-2xl font-medium text-teal-300 tracking-tight mt-10 mb-4 first:mt-0",
        className
      )}
      {...props}
    />
    ...
    if (href?.startsWith("/") || href?.startsWith("#")) {
      return (
        <Link
          href={href}
          className={cn(
            "text-teal-300 font-medium no-underline hover:underline hover:underline-offset-4 transition-all duration-200",
            className
          )}
          {...props}
        />
  ...
  li: ({ className, ...props }: React.HTMLAttributes<HTMLLIElement>) => (
    <li
      className={cn(
        "leading-8 text-muted-foreground relative pl-6 before:content-['◇'] before:absolute before:left-0 before:text-green-400 before:font-mono before:text-sm",
        className
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => {
    ...
    if (isInline) {
      return (
        <code
          className={cn(
            "bg-zinc-900 text-teal-300 px-2 py-1 rounded text-sm font-mono border border-zinc-800",
            className
          )}
          {...props}
        />
  ...
  blockquote: ({
    ...
    <blockquote
      className={cn(
        "border-l-4 border-teal-300 pl-6 my-6 text-muted-foreground italic bg-muted/30 py-4 rounded-r-lg",
        className
      )}
      {...props}
    />
    ...
    if (type === "checkbox") {
      return (
        <input
          ...
          checked={checked}
          disabled={disabled}
          className={cn("mr-2 accent-teal-300 cursor-default", className)}
          {...props}
        />
      );
    }
